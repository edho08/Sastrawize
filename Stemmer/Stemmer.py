import re
from Sastrawize.Stemmer.Context.Visitor.VisitorProvider import VisitorProvider
from Sastrawize.Stemmer.Filter import TextNormalizer
from Sastrawize.Stemmer.Context.Context import Context
...
class StemExecutor:
    _func = None
    _executor_count = None
    _exe_num = None
    _input = None
    _output = None
    def __init__(self, func, exe_num, exe_count, inp, out):
        self._func = func
        self._executor_count = exe_count
        self._input = inp
        self._output = out
        self._exe_num = exe_num
    
    def execute(self):
        i = self._exe_num;
        l = len(self._input)
        while i < l:
            self.output[i] = self.func(input[i])
 '''       
